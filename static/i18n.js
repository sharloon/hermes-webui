...
const LOCALES = {
  en: {
    ...
    offline_network_detail: 'OpsMax is unreachable from this browser right now.',
    offline_autorefresh: 'I will refresh this page automatically when OpsMax is reachable again.',
    ...
    mcp_restart_hint: 'Server changes are read-only here for now. Edit config.yaml and restart OpsMax for changes to take effect.',
    ...
    cmd_skills: 'List available OpsMax skills',
  ...
  status_agent_running:'Agent running',
    ...
    status_hermes_home: 'OpsMax home',
  ...
  folder_add_as_space_title: 'Add as Space?',
    ...
    settings_section_preferences_meta: 'Defaults and UI behavior for OpsMax Web UI.',
    ...
    kanban_work_queue_hint: 'This is the OpsMax Agent work queue. Create or triage a task, assign it, move it to Ready, then let the dispatcher claim it.',
    ...
    kanban_assignee_profiles_label: 'OpsMax profiles',
    ...
    tab_dashboard: 'OpsMax Dashboard',
    ...
    settings_desc_voice_mode: 'Show the voice-mode button (audio waveform) next to the dictation mic. Lets you speak naturally — OpsMax auto-sends after a pause and reads replies aloud. Requires a browser that supports both speech recognition and TTS.',
    ...
    settings_desc_bot_name: 'Display name for the assistant throughout the UI. Defaults to OpsMax.',
    ...
    onboarding_title: 'Welcome to OpsMax Web UI',
    onboarding_lead: 'A quick guided setup will verify OpsMax, save a real provider configuration, choose a workspace and model, and optionally protect the app with a password.',
    onboarding_back: 'Back',
    onboarding_continue: 'Continue',
    onboarding_skip: 'Skip setup',
    onboarding_skipped: 'Setup skipped — using existing config.',
    onboarding_open: 'Open OpsMax',
    onboarding_step_system_title: 'System check',
    onboarding_step_system_desc: 'Verify OpsMax Agent and config visibility.',
    onboarding_step_setup_title: 'Provider setup',
    onboarding_step_setup_desc: 'Save the minimum OpsMax provider config.',
    ...
    onboarding_notice_system_ready: 'OpsMax Agent looks reachable from the Web UI.',
    onboarding_notice_system_unavailable: 'OpsMax Agent is not fully available yet. Bootstrap can install it, but provider setup may still require a terminal.',
    onboarding_check_agent: 'OpsMax Agent',
    ...
    onboarding_notice_setup_required: 'Choose a simple provider path here. Advanced OAuth flows still belong in the OpsMax CLI for now.',
    onboarding_notice_setup_already_ready: 'A working OpsMax provider setup is already detected. You can keep it or replace it here.',
    onboarding_oauth_provider_ready_title: 'Provider already authenticated',
    onboarding_oauth_provider_ready_body: 'This instance is configured to use an OAuth provider (<strong>{provider}</strong>) that was set up via the OpsMax CLI. No API key is needed here — click Continue to finish setup.',
    ...
    onboarding_api_key_help_prefix: 'Saved as a secret in your OpsMax .env file using',
    ...
    onboarding_workspace_help: 'Pick the model OpsMax should use for new chats after setup completes.',
  ...
  ja: {
    ...
    offline_network_detail: '現在、このブラウザからOpsMaxに到達できません。',
    offline_autorefresh: 'OpsMaxに再接続できたら、このページを自動的に更新します。',
    ...
    mcp_restart_hint: 'ここでのサーバー変更は現状読み取り専用です。反映するには config.yaml を編集して OpsMax を再起動してください。',
    ...
    cmd_skills: '利用可能な OpsMax スキルを一覧表示',
  ...
  folder_add_as_space_title: 'スペースとして追加?',
    ...
    settings_section_preferences_meta: 'OpsMax Web UI のデフォルトと UI 動作。',
    ...
    kanban_work_queue_hint: 'これは OpsMax Agent のワークキューです。タスクを作成またはトリアージし、担当者を割り当て、Ready に移動すると、ディスパッチャーがそれをクレームします。',
    ...
    kanban_assignee_profiles_label: 'OpsMax プロファイル',
    ...
    settings_desc_bot_name: 'UI 全体で表示されるアシスタントの名前。デフォルトは OpsMax。',
    ...
    onboarding_title: 'OpsMax Web UI へようこそ',
    onboarding_lead: 'クイックガイドで OpsMax を確認し、プロバイダ設定を保存し、ワークスペースとモデルを選択し、必要ならパスワードでアプリを保護します。',
    onboarding_back: '戻る',
    onboarding_continue: '続ける',
    onboarding_skip: 'セットアップをスキップ',
    onboarding_skipped: 'セットアップをスキップ — 既存設定を使用します。',
    onboarding_open: 'OpsMax を開く',
    onboarding_step_system_title: 'システム確認',
    onboarding_step_system_desc: 'OpsMax Agent と設定の可視性を確認します。',
    onboarding_step_setup_title: 'プロバイダ設定',
    onboarding_step_setup_desc: '最低限の OpsMax プロバイダ設定を保存します。',
    ...
    onboarding_notice_system_ready: 'Web UI から OpsMax Agent へ到達できそうです。',
    onboarding_notice_system_unavailable: 'OpsMax Agent はまだ完全には利用できません。Bootstrap でインストールできますが、プロバイダ設定にはターミナルが必要な場合があります。',
    onboarding_check_agent: 'OpsMax Agent',
    ...
    onboarding_notice_setup_required: 'シンプルなプロバイダ経路をここで選んでください。高度な OAuth フローは現状 OpsMax CLI 側で扱います。',
    onboarding_notice_setup_already_ready: '動作する OpsMax プロバイダ設定が既に検出されています。維持するか、ここで置き換えできます。',
    onboarding_oauth_provider_ready_title: 'プロバイダは認証済み',
    onboarding_oauth_provider_ready_body: 'このインスタンスは OpsMax CLI で設定された OAuth プロバイダ (<strong>{provider}</strong>) を使用します。APIキーは不要 — 続けるをクリックしてセットアップを完了してください。',
    ...
    onboarding_api_key_help_prefix: 'OpsMax の .env ファイルにシークレットとして保存されます — 使用変数:',
    ...
    onboarding_workspace_help: 'セットアップ完了後の新規チャットで OpsMax が使用するモデルを選択してください。',
    onboarding_custom_model_placeholder: 'your-model-name',
  ...
  ru: {
    ...
    offline_network_detail: 'OpsMax сейчас недоступен из этого браузера.',
    offline_autorefresh: 'Я автоматически обновлю страницу, когда OpsMax снова станет доступен.',
    ...
    cmd_skills: 'Показать доступные навыки OpsMax',
  ...
  folder_add_as_space_title: 'Добавить как пространство?',
    ...
    kanban_assignee_profiles_label: 'Профили OpsMax',
    ...
    settings_desc_sync_insights: 'Синхронизирует использование токенов WebUI в state.db, чтобы OpsMax /insights включал данные браузерных сеансов. Выключено по умолчанию.',
    ...
    settings_desc_bot_name: 'Отображаемое имя помощника во всём интерфейсе. По умолчанию OpsMax.',
    ...
    onboarding_title: 'Добро пожаловать в OpsMax Web UI',
    onboarding_lead: 'Краткая пошаговая настройка проверит OpsMax, сохранит рабочую конфигурацию провайдера, выберет рабочее пространство и модель и при желании защитит приложение паролем.',
    ...
    onboarding_open: 'Открыть OpsMax',
    onboarding_step_system_title: 'Проверка системы',
    onboarding_step_system_desc: 'Проверить OpsMax Agent и видимость конфигурации.',
    onboarding_step_setup_title: 'Настройка провайдера',
    onboarding_step_setup_desc: 'Сохранить минимальную рабочую конфигурацию провайдера OpsMax.',
    ...
    onboarding_notice_system_ready: 'OpsMax Agent, похоже, доступен из Web UI.',
    onboarding_notice_system_unavailable: 'OpsMax Agent ещё не полностью доступен. Bootstrap может установить его, но для настройки провайдера всё ещё может понадобиться терминал.',
    onboarding_check_agent: 'OpsMax Agent',
    ...
    onboarding_notice_setup_required: 'Выберите здесь простой путь настройки провайдера. Продвинутые OAuth-сценарии пока остаются в OpsMax CLI.',
    onboarding_notice_setup_already_ready: 'Уже обнаружена рабочая конфигурация провайдера OpsMax. Вы можете оставить её или заменить здесь.',
    onboarding_oauth_provider_ready_title: 'Провайдер уже авторизован',
    onboarding_oauth_provider_ready_body: 'Этот экземпляр настроен на использование OAuth-провайдера (<strong>{provider}</strong>), настроенного через OpsMax CLI. API-ключ здесь не нужен — нажмите «Продолжить», чтобы завершить настройку.',
    ...
    onboarding_api_key_help_prefix: 'Сохраняется как секрет в вашем файле `.env` OpsMax с помощью',
    ...
    onboarding_workspace_help: 'Выберите модель, которую OpsMax должен использовать для новых чатов после завершения настройки.',
    ...
    settings_desc_voice_mode: 'Показывать кнопку голосового режима (аудиоволны) рядом с микрофоном диктовки. OpsMax автоматически отправляет реплики после паузы и зачитывает ответы вслух. Требуется браузер с поддержкой распознавания речи и TTS.',
  ...
  es: {
    ...
    offline_network_detail: 'OpsMax no está disponible desde este navegador ahora mismo.',
    offline_autorefresh: 'Actualizaré esta página automáticamente cuando OpsMax vuelva a estar disponible.',
    ...
    cmd_skills: 'Listar las skills de OpsMax disponibles',
  ...
  folder_add_as_space_title: '¿Añadir como espacio?',
    ...
    kanban_assignee_profiles_label: 'Perfiles OpsMax',
    ...
    settings_desc_bot_name: 'Nombre visible del asistente en toda la UI. Por defecto es OpsMax.',
    ...
    onboarding_title: 'Bienvenido a OpsMax Web UI',
    onboarding_lead: 'Una guía rápida verificará OpsMax, guardará una configuración real del proveedor, elegirá un espacio de trabajo y un modelo, y opcionalmente protegerá la app con una contraseña.',
    ...
    onboarding_open: 'Abrir OpsMax',
    onboarding_step_system_title: 'Comprobación del sistema',
    onboarding_step_system_desc: 'Verifica OpsMax Agent y la visibilidad de la configuración.',
    onboarding_step_setup_title: 'Configuración del proveedor',
    onboarding_step_setup_desc: 'Guarda la configuración mínima real de OpsMax.',
    ...
    onboarding_notice_system_ready: 'Parece que OpsMax Agent está accesible desde la Web UI.',
    onboarding_notice_system_unavailable: 'OpsMax Agent todavía no está totalmente disponible. Bootstrap puede instalarlo, pero la configuración del proveedor quizá aún requiera una terminal.',
    onboarding_check_agent: 'OpsMax Agent',
    ...
    onboarding_notice_setup_required: 'Elige aquí una ruta simple de proveedor. Los flujos OAuth avanzados siguen siendo del CLI de OpsMax por ahora.',
    onboarding_notice_setup_already_ready: 'Ya se detectó una configuración funcional del proveedor de OpsMax. Puedes conservarla o reemplazarla aquí.',
    onboarding_oauth_provider_ready_title: 'Proveedor ya autenticado',
    onboarding_oauth_provider_ready_body: 'Esta instancia está configurada para usar un proveedor OAuth (<strong>{provider}</strong>) configurado mediante la CLI de OpsMax. No se necesita clave API aquí — haz clic en Continuar para finalizar la configuración.',
    ...
    oauth_codex_error: 'OAuth login failed', // TODO: translate
    oauth_codex_expired: 'Code expired, please try again', // TODO: translate
    onboarding_api_key_help_prefix: 'Se guarda como secreto en tu archivo .env de OpsMax usando',
    onboarding_base_url_label: 'Base URL',
    ...
    onboarding_workspace_help: 'Elige el modelo que OpsMax debe usar para nuevos chats cuando termine la configuración.',
    ...
    settings_desc_voice_mode: 'Show the voice-mode button (audio waveform) next to the dictation mic. Lets you speak naturally — OpsMax auto-sends after a pause and reads replies aloud. Requires a browser that supports both speech recognition and TTS.',  // TODO: translate
  ...
  de: {
    ...
    offline_network_detail: 'OpsMax ist von diesem Browser aus gerade nicht erreichbar.',
    offline_autorefresh: 'Ich aktualisiere diese Seite automatisch, sobald OpsMax wieder erreichbar ist.',
    ...
    cmd_skills: 'Verfügbare OpsMax-Skills auflisten',
  ...
  folder_add_as_space_title: 'Als Bereich hinzufügen?',
    ...
    kanban_assignee_profiles_label: 'OpsMax-Profile',
    ...
    settings_desc_bot_name: 'Anzeigename für den Assistenten in der UI. Standardmäßig OpsMax.',
    ...
    onboarding_title: 'Willkommen bei OpsMax',
    ...
    onboarding_step_system_desc: 'OpsMax Agent muss installiert und erreichbar sein.',
    ...
    onboarding_notice_system_unavailable: 'Agent nicht erreichbar. Bitte starten Sie OpsMax Agent.',
  ...
  zh: {
    ...
    offline_network_detail: '此浏览器当前无法连接到 OpsMax。',
    offline_autorefresh: '当 OpsMax 可访问时，我会自动刷新此页面。',
    ...
    mcp_restart_hint: '服务器更改目前为只读。请编辑 config.yaml 并重启 OpsMax。',
  ...
  folder_add_as_space_title: '添加为工作区？',
    ...
    kanban_work_queue_hint: '这是 OpsMax Agent 的工作队列。创建或分类任务，分配任务，将其移至“就绪”状态，然后让调度员认领任务。',
    ...
    kanban_assignee_profiles_label: 'OpsMax 配置',
    ...
    settings_desc_bot_name: '助手在 UI 中的显示名称。默认为 OpsMax。',
    settings_desc_password: '输入新密码以设置或更改。留空保持当前设置。',
    // onboarding
    onboarding_badge: '首次运行',
    onboarding_title: '欢迎使用 OpsMax Web UI',
    archive_extracted: (n, c) => `从 ${c} 个压缩包中解压了 ${n} 个文件`,
    onboarding_lead: '快速引导将验证 OpsMax、保存真实的提供商配置、选择工作区和模型，并可选设置密码保护应用。',
    onboarding_back: '返回',
    onboarding_continue: '继续',
    onboarding_skip: '跳过设置',
    onboarding_skipped: '设置已跳过 — 使用现有配置。',
    onboarding_open: '打开 OpsMax',
    onboarding_step_system_title: '系统检查',
    onboarding_step_system_desc: '验证 OpsMax Agent 与配置可见性。',
    onboarding_step_setup_title: '提供商设置',
    onboarding_step_setup_desc: '保存最小可用的 OpsMax 提供商配置。',
    ...
    onboarding_notice_system_ready: 'OpsMax Agent 看起来可从 Web UI 访问。',
    onboarding_notice_system_unavailable: 'OpsMax Agent 尚未完全可用。Bootstrap 可以安装它，但提供商设置可能仍需要终端。',
    onboarding_check_agent: 'OpsMax Agent',
    ...
    onboarding_notice_setup_required: '请先在此选择一个简单的提供商路径。高级 OAuth 流程暂时仍建议在 OpsMax CLI 中完成。',
    onboarding_notice_setup_already_ready: '已检测到可用的 OpsMax 提供商配置。你可以保留它，或在这里替换。',
    onboarding_oauth_provider_ready_title: '提供商已完成认证',
    onboarding_oauth_provider_ready_body: '此实例已配置为使用通过 OpsMax CLI 设置的 OAuth 提供商（<strong>{provider}</strong>）。这里不需要 API key，点击继续即可完成设置。',
    ...
    onboarding_api_key_help_prefix: '会作为密钥保存到 OpsMax .env 文件中，变量名为',
    onboarding_base_url_label: 'Base URL',
    onboarding_base_url_placeholder: 'https://your-endpoint.example/v1',
    onboarding_base_url_help: '用于 OpenAI 兼容路由、自托管服务、LiteLLM、Ollama、LM Studio、vLLM 或类似端点。',
    onboarding_model_label: '默认模型',
    onboarding_workspace_help: '选择设置完成后 OpsMax 在新聊天中使用的模型。',
    onboarding_custom_model_placeholder: 'your-model-name',
    ...
    settings_section_preferences_meta: 'OpsMax WebUI 的默认和 UI 行为。',
    ...
    settings_desc_voice_mode: '在听写麦克风旁显示语音模式按钮（音频波形）。让您自然说话 — OpsMax 会在停顿后自动发送并朗读回复。需要支持语音识别和 TTS 的浏览器。',
  ...
  folder_add_as_space_title: '\u65b0\u589e\u70ba\u5de5\u4f5c\u5340\uff1f',
    ...
    settings_section_preferences_meta: 'OpsMax Web UI 的預設值與介面行為。',
    ...
    kanban_work_queue_hint: '這是 OpsMax Agent 的工作佇列。建立或分類任務、指派對象、移至 Ready，然後讓調度器認領。',
    ...
    kanban_assignee_profiles_label: 'OpsMax 設定檔',
    ...
    session_jump_end: '結尾',
    session_jump_end_label: '跳至會話結尾',
    onboarding_api_key_help_prefix: '\u900f\u904e\u4ee5\u4e0b\u65b9\u5f0f\u5132\u5b58\u70ba OpsMax .env \u6a94\u6848\u4e2d\u7684\u6a5f\u5bc6',
    ...
    onboarding_lead: '\u5feb\u901f\u9010\u6b65\u8a2d\u5b9a\uff1a\u6aa2\u67e5 OpsMax\u3001\u5132\u5b58\u63d0\u4f9b\u8005\u914d\u7f6e\u3001\u9078\u64c7\u5de5\u4f5c\u5340\u8207\u6a21\u578b\uff0c\u4e26\u53ef\u9078\u64c7\u52a0\u5165\u5bc6\u78bc\u4fdd\u8b77\u3002',
    ...
    onboarding_notice_setup_already_ready: '\u5df2\u5075\u6e2c\u5230\u53ef\u7528\u7684 OpsMax \u63d0\u4f9b\u8005\u914d\u7f6e\u3002\u60a8\u53ef\u4ee5\u4fdd\u7559\u6216\u5728\u6b64\u53d6\u4ee3\u3002',
    onboarding_notice_setup_required: '\u5728\u6b64\u9078\u64c7\u5feb\u901f\u63d0\u4f9b\u8005\u8a2d\u5b9a\u8def\u5f91\u3002\u9032\u968e OAuth \u8a2d\u5b9a\u4ecd\u8acb\u4f7f\u7528 OpsMax CLI\u3002',
    onboarding_notice_system_ready: 'OpsMax Agent \u53ef\u5f9e Web UI \u5b58\u53d6\u3002',
    onboarding_notice_system_unavailable: 'OpsMax Agent \u5c1a\u672a\u5b8c\u5168\u53ef\u7528\u3002\u5f15\u5c0e\u7a0b\u5f0f\u53ef\u80fd\u5b89\u88dd\u5b83\uff0c\u4f46\u63d0\u4f9b\u8005\u8a2d\u5b9a\u53ef\u80fd\u4ecd\u9700\u7d42\u7aef\u6a5f\u3002',
    ...
    onboarding_oauth_provider_ready_body: '\u6b64\u5be6\u4f8b\u8a2d\u5b9a\u4f7f\u7528\u900f\u904e OpsMax CLI \u914d\u7f6e\u7684 OAuth \u63d0\u4f9b\u8005\uff08<strong>{provider}</strong>\uff09\u3002\u7121\u9700 API \u91d1\u9470 \u2014 \u6309\u7e7c\u7e8c\u5b8c\u6210\u8a2d\u5b9a\u3002',
    onboarding_oauth_provider_ready_title: '\u63d0\u4f9b\u8005\u5df2\u6388\u6b0a',
    onboarding_oauth_switch_hint: '\u6216\u9078\u64c7\u4e0b\u65b9\u5176\u4ed6\u63d0\u4f9b\u8005\u4ee5\u5207\u63db\u5230 API \u91d1\u9470\u8a2d\u5b9a\uff1a',
    onboarding_open: '\u958b\u555f OpsMax',
    ...
    onboarding_step_setup_desc: '\u5132\u5b58\u6700\u5c0f\u53ef\u7528\u7684 OpsMax \u63d0\u4f9b\u8005\u914d\u7f6e\u3002',
    onboarding_step_setup_title: '\u63d0\u4f9b\u8005\u8a2d\u5b9a',
    onboarding_step_system_desc: '\u9a57\u8b49 OpsMax Agent \u8207\u914d\u7f6e\u53ef\u898b\u6027\u3002',
    onboarding_step_system_title: '\u7cfb\u7d71\u6aa2\u67e5',
    onboarding_step_workspace_desc: '\u70ba\u65b0\u6703\u8a71\u9078\u64c7\u9810\u8a2d\u503c\u3002',
    onboarding_step_workspace_title: '\u5de5\u4f5c\u5340\u8207\u6a21\u578b',
    onboarding_title: '\u6b61\u8fce\u4f7f\u7528 OpsMax Web UI',
    onboarding_unknown: '\u672a\u77e5',
    onboarding_workspace_help: '\u9078\u64c7 OpsMax \u5728\u8a2d\u5b9a\u5b8c\u6210\u5f8c\u7528\u65bc\u65b0\u804a\u5929\u5ba4\u7684\u6a21\u578b\u3002',
  ...
  pt: {
    ...
    offline_network_detail: 'O OpsMax está inacessível neste navegador agora.',
    offline_autorefresh: 'Vou atualizar esta página automaticamente quando o OpsMax voltar a responder.',
    ...
    cmd_skills: 'Listar skills disponíveis do OpsMax',
    ...
    status_hermes_home: 'Diretório OpsMax',
    ...
    settings_section_preferences_meta: 'Padrões e comportamento UI do OpsMax Web UI.',
    ...
    kanban_assignee_profiles_label: 'Perfis OpsMax',
    ...
    settings_desc_bot_name: 'Nome de exibição do assistente. Padrão: OpsMax.',
    ...
    onboarding_title: 'Bem-vindo ao OpsMax Web UI',
    onboarding_lead: 'Uma configuração rápida vai verificar OpsMax, salvar provedor, escolher workspace e modelo, e opcionalmente proteger com senha.',
    ...
    onboarding_step_system_desc: 'Verificar OpsMax Agent e visibilidade da config.',
    onboarding_step_setup_title: 'Configuração do provedor',
    onboarding_step_setup_desc: 'Salvar config mínima do provedor OpsMax.',
    ...
    onboarding_notice_system_ready: 'OpsMax Agent parece acessível pela Web UI.',
    onboarding_notice_system_unavailable: 'OpsMax Agent não está totalmente disponível. Bootstrap pode instalar, mas setup do provedor pode requerer terminal.',
    onboarding_check_agent: 'OpsMax Agent',
    ...
    onboarding_notice_setup_required: 'Escolha caminho simples de provedor aqui. OAuth avançado ainda pertence ao OpsMax CLI.',
    onboarding_notice_setup_already_ready: 'Setup de provedor OpsMax já detectado. Pode manter ou substituir.',
    ...
    onboarding_api_key_placeholder_optional: 'Leave blank for keyless servers', // TODO: translate
    onboarding_api_key_help_keyless: 'Most LM Studio / Ollama / vLLM installs run keyless — leave this blank if your server doesn\'t require authentication. Use the Test connection button to verify.', // TODO: translate
    onboarding_api_key_help_prefix: 'Salvo como segredo no .env do OpsMax usando',
    onboarding_base_url_label: 'Base URL',
    ...
    onboarding_workspace_help: 'Escolha modelo que OpsMax deve usar para novos chats.',
  ...
  ko: {
    ...
    offline_network_detail: '현재 이 브라우저에서 OpsMax에 연결할 수 없습니다.',
    offline_autorefresh: 'OpsMax에 다시 연결되면 이 페이지를 자동으로 새로고침합니다.',
    ...
    settings_section_preferences_meta: 'OpsMax Web UI의 기본값과 UI 동작.',
    ...
    kanban_assignee_profiles_label: 'OpsMax 프로필',
    ...
    settings_desc_bot_name: 'UI 전체에 표시되는 Assistant 이름입니다. 기본값은 OpsMax입니다.',
    ...
    onboarding_title: 'OpsMax Web UI에 오신 것을 환영합니다',
    onboarding_lead: '간단한 안내 설정으로 OpsMax 설치를 확인하고, 워크스페이스와 모델을 선택하며, 필요하면 비밀번호로 앱을 보호합니다.',
    onboarding_back: '뒤로',
